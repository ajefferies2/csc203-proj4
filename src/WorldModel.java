    ...
    */
    ...
    private void parseSapling(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Sapling.NUM_PROPERTIES) {
            int health = Integer.parseInt(properties[Sapling.HEALTH_IDX]);
            Entity entity = new Sapling(id, pt, imageStore.getImageList(Sapling.KEY), health);
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Sapling.KEY, Sapling.NUM_PROPERTIES));
        }
    }
    ...
    private void parseDude(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Dude.NUM_PROPERTIES) {
            Entity entity = new DudeNotFull(id, pt, Double.parseDouble(properties[Dude.ACTION_PERIOD_IDX]), Double.parseDouble(properties[Dude.ANIMATION_PERIOD_IDX]), Integer.parseInt(properties[Dude.RESOURCE_LIMIT_IDX]), imageStore.getImageList(Dude.KEY));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Dude.KEY, Dude.NUM_PROPERTIES));
        }
    }
    ...
    private void parseFairy(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Fairy.NUM_PROPERTIES) {
            Entity entity = new Fairy(id, pt, Double.parseDouble(properties[Fairy.ACTION_PERIOD_IDX]), Double.parseDouble(properties[Fairy.ANIMATION_PERIOD_IDX]), imageStore.getImageList(Fairy.KEY));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Fairy.KEY, Fairy.NUM_PROPERTIES));
        }
    }
    ...
    private void parseTree(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Tree.NUM_PROPERTIES) {
            Entity entity = new Tree(id, pt, Double.parseDouble(properties[Tree.ACTION_PERIOD_IDX]), Double.parseDouble(properties[Tree.ANIMATION_PERIOD_IDX]), Integer.parseInt(properties[Tree.HEALTH_IDX]), imageStore.getImageList(Tree.KEY));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Tree.KEY, Tree.NUM_PROPERTIES));
        }
    }
    ...
    private void parseObstacle(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Obstacle.NUM_PROPERTIES) {
            Entity entity = new Obstacle(id, pt, Double.parseDouble(properties[Obstacle.ANIMATION_PERIOD_IDX]), imageStore.getImageList(Obstacle.KEY));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Obstacle.KEY, Obstacle.NUM_PROPERTIES));
        }
    }
    ...
    private void parseHouse(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == House.NUM_PROPERTIES) {
            Entity entity = new House(id, pt, imageStore.getImageList(House.KEY));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", House.KEY, House.NUM_PROPERTIES));
        }
    }
    ...
    private void parseStump(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Stump.NUM_PROPERTIES) {
            Entity entity = new Stump(id, pt, imageStore.getImageList(Stump.KEY));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Stump.KEY, Stump.NUM_PROPERTIES));
        }
    }

    private void parseFire(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Wildfire.NUM_PROPERTIES) {
            Entity entity = new Fairy(id, pt, 1, 1, imageStore.getImageList(Wildfire.KEY));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Stump.KEY, Stump.NUM_PROPERTIES));
        }
    ...
    public void parseEntity(String line, ImageStore imageStore) {
        ...
        if (properties.length >= ENTITY_NUM_PROPERTIES) {
            ...
            switch (key) {
                case Obstacle.KEY -> this.parseObstacle(properties, pt, id, imageStore);
                case Dude.KEY -> this.parseDude(properties, pt, id, imageStore);
                case Fairy.KEY -> this.parseFairy(properties, pt, id, imageStore);
                case House.KEY -> this.parseHouse(properties, pt, id, imageStore);
                case Tree.KEY -> this.parseTree(properties, pt, id, imageStore);
